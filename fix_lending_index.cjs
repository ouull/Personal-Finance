...
export default function LendingScreen() {
  ...
  const lentLoans = loans.filter((l: any) => l.type === 'LENT');
      ...
      return (
        <TouchableOpacity key={loan.id} onPress={() => router.push(\`/lending/\${loan.id}\`)}>
          <Card className="mb-4 bg-transparent border border-[#E5E0D5] rounded-[24px] p-5 shadow-none">
            ...
            <Text className="text-gray-600 text-[13px] mb-0.5">{loan.notes || (loan.type === 'LENT' ? 'Loan' : 'Debt')}</Text>
            {loan.dueDate && (
            ...
            <View className="mt-2 mb-4">
              <View className="flex-row justify-between items-center mb-1.5">
                <Text className="text-gray-600 text-xs font-medium">Repayment Progress</Text>
            ...
            {loan.repayments && loan.repayments.length > 0 && (
              <View className="mt-2">
                <Text className="text-gray-900 font-bold text-sm mb-3">Recent Payments</Text>
                {loan.repayments.slice(0, 3).map((r: any, idx: number) => (
                  <View key={r.id} className={\`flex-row justify-between items-center py-2.5 \${idx !== Math.min(loan.repayments.length, 3) - 1 ? 'border-b border-[#E5E0D5]' : ''}\`}>
                    <View className="flex-row items-center flex-1 mr-2">
                      ...
                      <View>
                        <Text className="text-gray-800 text-[13px] font-medium mb-0.5">Payment received</Text>
  ...
  return (
    <Screen safeArea={false}>
      <View className="flex-row justify-between items-center px-4 pt-12 pb-4 bg-[#FDF8EB]">
        <View className="flex-row items-center w-8">
          <TouchableOpacity onPress={() => router.push('/(tabs)/')} className="w-10 h-10 rounded-full bg-[#EAE5D9] items-center justify-center">
            <User size={20} color="#111827" />
          </TouchableOpacity>
        </View>
        <Text className="text-base font-medium text-gray-900 flex-1 text-center">Lending & Debts</Text>
        ...
        <View className="flex-row justify-between space-x-2 mb-6">
          <Card className="flex-1 bg-transparent border border-[#E5E0D5] p-4 rounded-[16px] shadow-none mr-2">
            <View className="flex-row items-center mb-2">
              ...
              <Text className="text-gray-600 font-medium text-[13px]">Berhutang Ke saya</Text>
            </View>
          ...
          <Card className="flex-1 bg-transparent border border-[#E5E0D5] p-4 rounded-[16px] shadow-none ml-2">
            <View className="flex-row items-center mb-2">
              ...
              <Text className="text-gray-600 font-medium text-[13px]">Hutang Saya</Text>
            </View>
...
`;
fs.writeFileSync('mobile/app/lending/index.tsx', content, 'utf8');
