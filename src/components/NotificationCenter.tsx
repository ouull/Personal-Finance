...
import { getNotifications, markNotificationAsRead, markAllNotificationsAsRead } from "@/features/notifications/actions"
      ...
      <PopoverContent align="end" className="w-80 p-0 rounded-2xl shadow-xl overflow-hidden">
        <div className="flex items-center justify-between p-4 border-b border-slate-100 bg-slate-50/50">
          <div className="flex items-center gap-2">
            ...
            {unreadCount > 0 && (
              <Badge variant="secondary" className="bg-indigo-100 text-indigo-700 hover:bg-indigo-100">{unreadCount}</Badge>
            )}
          </div>
          {unreadCount > 0 && (
            <Button variant="ghost" size="sm" onClick={handleMarkAll} className="h-8 text-xs text-slate-500 hover:text-indigo-600">
              ...
              {notifications.map((n) => (
                ...
                >
                  <div className="flex justify-between items-start gap-2">
                    <div>
                      <h4 className={`text-sm ${!n.readAt ? "font-semibold text-slate-800" : "font-medium text-slate-600"}`}>
                        {n.title}
                      </h4>
