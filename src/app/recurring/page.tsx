...
export default async function RecurringPage() {
  ...
  const payments = JSON.parse(
    JSON.stringify(paymentsResult.success ? paymentsResult.data || [] : []),
  );
  const accounts = JSON.parse(
    JSON.stringify(accountsResult.success ? accountsResult.data || [] : []),
  );
  const categories = JSON.parse(
    JSON.stringify(categoriesResult.success ? categoriesResult.data || [] : []),
  );

  const activeSubscriptionsCount = payments.filter(
  ...
  const monthlyCommitments = payments.reduce((sum: number, p: any) => {
    let monthlyAmount = Number(p.amount);
    if (p.billingCycle === "WEEKLY") monthlyAmount = Number(p.amount) * 4.33;
    if (p.billingCycle === "QUARTERLY") monthlyAmount = Number(p.amount) / 3;
    if (p.billingCycle === "YEARLY") monthlyAmount = Number(p.amount) / 12;
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-200/50 pb-6">
          <div>
            <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight text-slate-900 pb-1">
              {t.recurringPage?.title || "Recurring"}
            </h1>
            <p className="text-muted-foreground mt-1 text-base">
              {t.recurringPage?.description ||
                "Manage your bills, subscriptions, and recurring expenses."}
            </p>
          </div>
        </header>
      </FadeIn>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-white p-5 rounded-2xl border border-slate-200/60 shadow-sm flex flex-col justify-between">
            <div className="text-sm font-medium text-slate-500 flex items-center gap-2">
              ...
              {t.recurringPage?.monthlyCommitments || "Monthly Commitments"}
            </div>
            <div className="text-2xl font-bold text-slate-900 mt-2">
              ~<CurrencyDisplay amount={monthlyCommitments} />
            </div>
          </div>
          ...
          <div className="bg-white p-5 rounded-2xl border border-amber-200 shadow-sm flex flex-col justify-between">
            <div className="text-sm font-medium text-amber-600 flex items-center gap-2">
              ...
              {t.recurringPage?.dueThisWeek || "Due This Week"}
            </div>
            <div className="text-2xl font-bold text-amber-600 mt-2">
              <CurrencyDisplay amount={dueThisWeek} />
            </div>
          </div>
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200/60 shadow-sm flex flex-col justify-between">
            <div className="text-sm font-medium text-slate-500 flex items-center gap-2">
              ...
              {t.recurringPage?.activeSubscriptions || "Active Subscriptions"}
            </div>
            <div className="text-2xl font-bold text-slate-900 mt-2">
