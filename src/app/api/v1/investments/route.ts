  ...
  try {
    ...
    const summary = investments.reduce(
      (acc, curr) => ({
        totalInvested:
          acc.totalInvested +
          (curr.status === "ACTIVE" ? curr.totalInvested : 0),
        currentValue:
          acc.currentValue + (curr.status === "ACTIVE" ? curr.currentValue : 0),
        realizedGain: acc.realizedGain + curr.realizedGain,
        unrealizedGain:
          acc.unrealizedGain +
          (curr.status === "ACTIVE" ? curr.unrealizedGain : 0),
      }),
    ...
    return NextResponse.json({
      success: true,
      data: { items: investments, summary },
    });
  } catch (error: any) {
    return NextResponse.json(
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { name, type, platform, notes, initialAmount, accountId } =
      parsed.data;
    ...
    const serialized = {
      ...
      unrealizedGain:
        Number(investment.currentValue) - Number(investment.totalInvested),
