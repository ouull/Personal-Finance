...
export async function createTransaction(userId: string, parsed: any) {
  return await db.$transaction(async (tx: any) => {
    ...
    if (parsed.destinationAccountId) {
      ...
      if (!destAcc || destAcc.userId !== userId) throw new TransactionError("UNAUTHORIZED", "Unauthorized destination account")
    }
    ...
    if (parsed.categoryId && (parsed.type === "INCOME" || parsed.type === "EXPENSE")) {
      ...
      if (!category || category.userId !== userId) throw new TransactionError("UNAUTHORIZED", "Unauthorized category")
      if (category.type !== parsed.type) {
        throw new TransactionError("CATEGORY_TYPE_MISMATCH", "category_type_mismatch")
      }
    }
...
export async function updateTransaction(userId: string, id: string, parsed: any) {
  return await db.$transaction(async (tx: any) => {
    // 1. Fetch original transaction and check ownership
    const originalTx = await tx.transaction.findUnique({ where: { id } })
    if (!originalTx || originalTx.userId !== userId) throw new TransactionError("UNAUTHORIZED", "Unauthorized transaction")
...
export async function getTransactions(userId: string, limit = 50, offset = 0, type?: string) {
...
export async function getTransactionById(userId: string, id: string) {
  ...
  if (!tx || tx.userId !== userId) {
    throw new TransactionError("UNAUTHORIZED", "Transaction not found or unauthorized")
  }
