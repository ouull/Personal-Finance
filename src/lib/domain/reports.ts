...
export async function getSpendingReport(userId: string, month: number, year: number) {
  ...
  const validCategoryIds = expenses.map(e => e.categoryId).filter(id => id !== null) as string[]
