...
export function RecurringPaymentList({
  ...
  const handleProcess = async (id: string, name: string) => {
    if (
      !confirm(
        t.common?.confirmDelete ||
          `Are you sure you want to mark ${name} as paid? This will deduct from your account.`,
      )
    )
      return;
    ...
    if (result.success) {
      ...
      toast.error(
        result.error || t.common?.error || `Failed to process ${name}`,
      );
    }
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center p-12 bg-white rounded-3xl border border-slate-200/50 shadow-sm text-center">
        ...
        <h3 className="text-lg font-bold text-slate-800 mb-2">
          {t.recurringPage?.noRecurring || "No recurring payments yet"}
        </h3>
        <p className="text-slate-500 mb-6 max-w-sm">
          {t.recurringPage?.empty ||
            "Set up your subscriptions, bills, and other regular payments to track them automatically."}
        </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {payments.map((payment) => {
          ...
          return (
            <div
              key={payment.id}
              className="bg-white p-5 rounded-2xl border border-slate-200/60 shadow-sm flex flex-col hover:shadow-md transition-shadow"
            >
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {payment.status === "ACTIVE" ? (
                      <Badge
                        variant="outline"
                        className="text-[10px] bg-emerald-50 text-emerald-700 border-emerald-200 uppercase"
                      >
                        Active
                      </Badge>
                    ) : (
                      <Badge
                        variant="outline"
                        className="text-[10px] bg-slate-50 text-slate-600 border-slate-200 uppercase"
                      >
                        Paused
                      </Badge>
                    )}
              ...
              <div className="mt-4 space-y-3 flex-1">
                ...
                <div
                  className={`mt-2 p-3 rounded-xl border ${statusColor} flex justify-between items-center`}
                >
