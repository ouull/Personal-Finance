...
export async function getInvestmentById(id: string) {
  try {
    const userId = await getUserId();
    const investment = await domain.getInvestmentById(userId, id);
...
export async function getInvestmentTransactions(id: string) {
  try {
    const userId = await getUserId();
    const transactions = await domain.getInvestmentTransactions(userId, id);
...
export async function addInvestmentTransaction(data: {
  ...
  try {
    ...
    if (
      ...
      error.message === "Insufficient investment cash balance"
    ) {
      return { success: false, error: "Saldo tidak mencukupi." };
    }
