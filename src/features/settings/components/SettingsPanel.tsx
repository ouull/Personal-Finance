    ...
    <div className="space-y-8">
      ...
      <section className="bg-white p-6 rounded-2xl border border-slate-200/60 shadow-sm">
        <div className="flex items-start gap-4 mb-6">
          ...
          <div>
            <h2 className="text-xl font-bold tracking-tight text-slate-800">{t.language}</h2>
            <p className="text-sm text-slate-500 mt-1">{t.languageDesc}</p>
          </div>
        </div>

        <div className="max-w-xs">
          <Select value={language} onValueChange={(val: any) => handleLanguageChange(val)} disabled={isSavingLang}>
            <SelectTrigger>
      ...
      <section className="bg-red-50/50 p-6 rounded-2xl border border-red-200 shadow-sm">
        <div className="flex items-start gap-4 mb-6">
          ...
          <div>
            <h2 className="text-xl font-bold tracking-tight text-red-800">{t.dangerZone}</h2>
        ...
        >
          {isResetting ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <AlertTriangle className="w-4 h-4 mr-2" />}
