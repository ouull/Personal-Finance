...
export function GoalList({ goals }: { goals: Goal[] }) {
  ...
  async function handleDelete(id: string) {
    if (!confirm(t.common?.confirmDelete || "Are you sure you want to delete this goal?")) return
    setIsDeleting(id)
  ...
  return (
    <div className="space-y-4">
      {goals.map((goal) => {
        ...
        return (
          <Card key={goal.id} className="bg-white/60 backdrop-blur-md border-white/50 hover:bg-white/90 hover:shadow-md hover:-translate-y-0.5 transition-all duration-300 shadow-sm group">
            <CardHeader className="pb-2 flex flex-row justify-between items-start space-y-0">
              <div className="space-y-1">
                ...
                <div className="text-xs font-medium text-slate-500">{percentage}% Achieved</div>
              </div>
