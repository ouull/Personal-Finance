...
export function AddCashDialog({ accountId }: AddCashDialogProps) {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    const res = await createTransaction({
      ...
      description:
        cashType === "INITIAL" ? "Saldo Awal Tunai" : "Pemasukan Tunai Baru",
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>
            {t.accountsPage?.addCashTitle || "Add Cash Balance"}
          </DialogTitle>
          <DialogDescription>
            {t.accountsPage?.addCashDesc ||
              "Record newly received cash or establish your opening cash balance."}
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={onSubmit} className="space-y-4 pt-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              >
                ...
                <span className="font-semibold text-sm">
                  {t.accountsPage?.typeInitial || "Opening Balance"}
                </span>
              </button>
              <button
                type="button"
              ...
              >
                ...
                <span className="font-semibold text-sm">
                  {t.accountsPage?.typeNewIncome || "New Income"}
                </span>
              </button>
            </div>
          </div>
          ...
          <div className="grid gap-2">
            <Label htmlFor="amount">
              {t.accountsPage?.amountToProceed || "Amount"}
            </Label>
            <Input
              id="amount"
              type="text"
