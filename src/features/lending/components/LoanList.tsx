...
export function LoanList({ loans, accounts }: LoanListProps) {
  ...
  const filteredLoans = loans.filter(
    (l) => filterType === "ALL" || (l.type || "LENT") === filterType,
  );
    ...
    return (
      <div className="flex flex-col items-center justify-center p-12 bg-white rounded-3xl border border-slate-200/50 shadow-sm text-center">
        ...
        <h3 className="text-lg font-bold text-slate-800 mb-2">
          {t.lendingPage?.noLoans || "No outstanding loans"}
        </h3>
        <p className="text-slate-500 mb-6 max-w-sm">
          {t.lendingPage?.empty ||
            "You haven't lent money to anyone yet. When you do, it will show up here."}
        </p>
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <h3 className="font-semibold text-slate-800">
          {t.lendingPage?.yourLoans || "Daftar Pinjaman"}
        </h3>
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full sm:w-auto">
          <div className="flex bg-slate-100 p-1 rounded-xl flex-1 sm:flex-none justify-between sm:justify-start">
            <button
      ...
      {filteredLoans.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredLoans.map((loan) => (
            <LoanDetailsDialog key={loan.id} loan={loan}>
              <div
                className="bg-white p-5 rounded-2xl border border-slate-200/60 shadow-sm flex flex-col hover:shadow-md transition-shadow h-full text-left"
              >
              <div className="flex justify-between items-start mb-3">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <Badge
                      variant="outline"
                      className={`text-[10px] uppercase tracking-wider ${(loan.type || "LENT") === "BORROWED" ? "bg-rose-50 text-rose-600 border-rose-200" : "bg-emerald-50 text-emerald-600 border-emerald-200"}`}
                    >
                      {(loan.type || "LENT") === "BORROWED"
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">
                    {t.lendingPage?.lentOn
                      ? `${t.lendingPage.lentOn} `
                      : "Lent on "}
                    {format(new Date(loan.lentDate), "d MMM yyyy")}
                  </p>
                </div>
              </div>
              ...
              <div className="mt-4 space-y-3 flex-1">
                <div className="flex justify-between items-center text-sm">
                  <span className="text-slate-500">
                    {t.lendingPage?.originalAmount || "Original Amount"}
                  </span>
                ...
                <div className="flex justify-between items-center text-sm">
                  <span className="text-slate-500">
                    {t.lendingPage?.alreadyPaid || "Already Paid"}
                  </span>
                  <span className="font-medium text-green-600">
                    {formatRupiah(loan.totalRepaid)}
                  </span>
                </div>
              ...
              {loan.status !== "PAID" && (
                <div className="mt-5 pt-4 border-t border-slate-100" onClick={(e) => e.stopPropagation()}>
